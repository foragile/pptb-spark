...
class KeySpace(cache: Cache, val keySpaceId: Int) {
  def get(datasetId: Any, partition: Int) = cache.get((keySpaceId, datasetId), partition)

  def put(datasetId: Any, partition: Int, value: Any) = cache.put((keySpaceId, datasetId), partition, value)
